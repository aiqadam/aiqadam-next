...
**Rework note (rework1):** TEST-RUNNER's `handoffs/WF02-REQ-038/step-04-test-runner.json` FAIL
found AC8's first `it()` in File 1 failing deterministically: `docs/agents/design/REQ-038.md`
line-wraps `resolution (a): the rule applies uniformly to\nevery` `requires_invite` event`
across lines 606-607 of the committed source, so the raw file never contains
`"resolution (a): the rule applies uniformly to every"` as one contiguous substring (a
newline sits where the assertion expected a space) — a test-fixture fragility against normal
markdown line-wrapping, not a defect in the design doc's content (the doc genuinely states
resolution (a), exactly as required, just soft-wrapped for readability) or the implementation.
Fixed by normalizing whitespace before the check: `designText.replace(/\s+/g, " ")` collapses
any run of whitespace (including a line-wrap newline) to a single space before both
`toContain` calls, so the assertion is now robust against any future rewrapping of this prose
while still requiring the exact phrase, in order, contiguous once wrapping is collapsed —
option (a) from TEST-RUNNER's filed issue, preferred over shortening the substring or
rewrapping the design doc because it doesn't weaken the check's intent and needs no future
maintenance if the doc's prose reflows again. No AC coverage change: still the same two
`toContain` assertions verifying the same two facts (resolution (a) is stated; no `events`
column is added). TEST-DESIGN-VALIDATOR gate not re-run — pure assertion-robustness fix, same
precedent as REQ-035's rework1 (`handoffs/WF02-REQ-035/step-03-test-designer-rework1.json`).
